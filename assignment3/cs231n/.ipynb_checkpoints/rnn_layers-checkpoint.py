...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = (x, Wx, prev_h, Wh, prev_c, next_c, i_gate, f_gate, o_gate, g_gate)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    x, Wx, prev_h, Wh, prev_c, next_c, i_gate, f_gate, o_gate, g_gate = cache
    N, H = dnext_h.shape
    
    dy = dnext_h * o_gate
    dnext_c += dy * ((1 + np.tanh(next_c)) * (1 - np.tanh(next_c)))
    ...
    di = di_gate * (i_gate * (1 - i_gate))
    df = df_gate * (f_gate * (1 - f_gate))
    do = do_gate * (o_gate * (1 - o_gate))
    dg = dg_gate * ((1 + g_gate) * (1 - g_gate))
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(T):
        x_input = x[:, i:i+1, :].reshape(N, D)
        next_h, next_c, step_cache = lstm_step_forward(x_input, prev_h, prev_c, Wx, Wh, b)
        h[:, i:i+1, :] = next_h.reshape(N, 1, H)
...
def lstm_backward(dh, cache):
    ...
    N, T, H = dh.shape
    x, Wx, prev_h, Wh, prev_c, next_c, i_gate, f_gate, o_gate, g_gate = cache[0]
    N, D = x.shape
    ...
    for i in range(T-1, -1, -1):
        step_cache = cache[i]
        dh_step = dh[:, i:i+1, :].reshape(N, H)
        
        dx_step, dprev_h, dprev_c, dWx_step, dWh_step, db_step = lstm_step_backward(dh_step + dnext_h, dnext_c, step_cache)
        dnext_c = dprev_c
        dnext_h = dprev_h
        
        dx[:, i:i+1, :] = dx_step.reshape(N, 1, D)
        dWx += dWx_step
        dWh += dWh_step
        db += db_step
